...
FVector ABoid::AvoidShips(TArray<AActor*> NearbyShips)
{
	...
	for(AActor* OverlapActor : NearbyShips)
	{
		...
		if(LocalShip != nullptr && LocalShip != this)
		{
			...
			SeperationDirection = GetActorLocation() = LocalShip->GetActorLocation();
			SeperationDirection = SeperationDirection.GetSafeNormal() * 2;

			// get scaling factor based off other ship's proximity. 0 = far away & 1 = very close
			ProximityFactor =  1.0f - (SeperationDirection.Size() / PerceptionSensor->GetScaledSphereRadius());

			// add steering force of ship and increase flock count
			Steering += (ProximityFactor * SeperationDirection);
			ShipCount++;
		}
	}
